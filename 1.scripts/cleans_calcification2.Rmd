---
title: "Cleans Calcification Data"
...
```{r}
bleach_raw <- read_excel(glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/updated sheets with meta data/Calcification Palmrya data repository.xlsx"), sheet = " bleaching tiles") %>% 
  clean_names()

acid_raw <- read_excel(glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/updated sheets with meta data/Calcification Palmrya data repository.xlsx"), sheet = "acid tiles") %>% 
  clean_names()

benthic_raw <- read_excel(glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/updated sheets with meta data/Calcification Palmrya data repository.xlsx"), sheet = "benthic data") %>% 
  clean_names()
...
sort(unique(bleach_raw$organic_mater_g))

bleach_clean <- bleach_raw %>% 
  rename(organic_matter = organic_mater_g,
...
```{r}
## Bleach
write_csv(bleach_clean, glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/clean_data/elmer_palmyra_calcification_bleaching_tiles_2013.csv"))

## Acid
write_csv(acid_clean, glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/clean_data/elmer_palmyra_calcification_acid_tiles_2013_2014.csv"))

## Benthic
write_csv(benthic_clean, glue("{dir_gdrive}/DATA_RAW/Franziska Elmer/clean_data/elmer_palmyra_calcification_benthic_coverage_2013_2014.csv"))
```
